...
class ActionExecutor(
    ...
    suspend fun request(action: JarvisAction): ActionResult {
        if (action.level != ActionLevel.SAFE) {
            ...
            return ActionResult(ActionStatus.NEEDS_USER, action.description, "Confirmation required before this ${if (action.level == ActionLevel.CRITICAL) "critical " else ""}action.")
        }
    ...
    suspend fun confirm(): ActionResult {
        val action = confirmationManager.takePending()
            ?: return ActionResult(ActionStatus.FAILED, "A pending action", "There is no pending action.", problem = "No pending action exists.")
        return executeAndVerify(action)
    }

    private suspend fun executeAndVerify(action: JarvisAction): ActionResult {
        state = ExecutorState.EXECUTING
        lastAction = action
        return try {
            if (action.allowed?.invoke() == false) {
                state = ExecutorState.IDLE
                return ActionResult(ActionStatus.FAILED, action.description, "The action is blocked by JARVIS permissions.", problem = "Permission policy denied the action.", cause = "Required access is no longer allowed.", fix = "Enable the required access and try again.")
            }
            ...
            state = ExecutorState.IDLE
            ActionResult(ActionStatus.FAILED, action.description, "The Android operation threw an error.", problem = "The action could not be completed.", cause = t.message ?: t::class.simpleName ?: "Unknown error", fix = "Check the required permission/app state and try again.")
        }
    }
    ...
    fun hasPending(): Boolean = confirmationManager.hasPendingAction()
    fun pendingDescription(): String? = confirmationManager.pendingDescription()
...
################################################################################
PATH: app/src/main/java/com/harsh/jarvis/ai/JarvisBrain.kt
################################################################################
...
class JarvisBrain(
    ...
    private suspend fun processSingle(input: String): BrainResponse {
        ...
        val response = when (result.intent) {
            ...
            IntentType.SAVE_MEMORY -> {
                ...
                val memoryText = run {
                    val marker = "remember"
                    val index = input.indexOf(marker, ignoreCase = true)
                    if (index >= 0) input.substring(index + marker.length).removePrefix("that").trim().ifBlank { input } else input
                }
            ...
            IntentType.SHOW_DEADLINES -> proactive?.refresh()?.let { events -> events.filter { it.deadline != null }.sortedBy { it.deadline }.take(5).joinToString("\n") { event -> "${event.title} — ${event.priority}" } } ?: "No deadlines are currently tracked."
            IntentType.SHOW_MISSING -> proactive?.refresh()?.let { events -> events.filter { it.status == "PENDING" }.take(5).joinToString("\n") { event -> "□ ${event.title}" } } ?: "Nothing is currently missing."
    ...
    private fun handleRoutine(input: String): String {
        val store = routines ?: return "Routine storage is not initialized."
        val match = Regex("""(?:create|save|make)\s+(?:a\s+)?routine\s+(.+?)\s+(?:with|:)\s+(.+)""", RegexOption.IGNORE_CASE).find(input.trim())
            ?: return "Tell me the routine name and commands, for example: create routine study with read notes; solve 5 problems."
        val name = match.groupValues[1].trim()
        val commands = match.groupValues[2].split(Regex("\\s*(?:;|\\|)\\s*"))
            .map { it.trim() }
            .filter { it.isNotBlank() }
        if (name.isBlank() || commands.isEmpty()) return "I need a routine name and at least one command."
        store.save(name, commands)
        return "Saved routine '$name' with ${commands.size} command(s)."
    }

    private fun handleProfile(input: String): String {
        val userProfile = profile ?: return "Profile is not initialized."
        val text = input.trim()
        val nameMatch = Regex("""(?:set|change)\s+(?:my\s+)?name\s*(?:to)?\s+(.+)""", RegexOption.IGNORE_CASE).find(text)
        if (nameMatch != null) {
            val value = nameMatch.groupValues[1].trim()
            if (value.isBlank()) return "Tell me the name you want me to use."
            userProfile.name = value
            return "Profile name updated."
        }
        val styleMatch = Regex("""(?:set|change)\s+(?:my\s+)?(?:assistant\s+)?style\s*(?:to)?\s+(.+)""", RegexOption.IGNORE_CASE).find(text)
        if (styleMatch != null) {
            val value = styleMatch.groupValues[1].trim().ifBlank { "balanced" }
            userProfile.assistantStyle = value
            return "Assistant style updated to '$value'."
        }
        return "Use 'set my name to ...' or 'set my style to balanced'."
    }
    ...
    fun hasPendingAction(): Boolean = executor.hasPending()
