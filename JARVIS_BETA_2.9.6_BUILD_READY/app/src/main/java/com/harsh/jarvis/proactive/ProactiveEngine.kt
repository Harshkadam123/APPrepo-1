...
class ProactiveEngine(
    ...
    suspend fun refresh(
        ...
        taskList
            ...
            .forEach { task ->
                ...
                val deadlineFactor =
                    PriorityEngine.deadlineFactor(deadline, now)
                ...
                val score = PriorityEngine.score(
                    ...
                    0.65 +
                        0.35 *
                        (100 - task.completionPercent.coerceIn(0, 100)) /
                        100.0
                    )

                upsert(
                    type = if (
                        task.title.contains("assignment", true)
                    ) {
                        "ASSIGNMENT"
                    } else {
                        "TASK"
    ...
    suspend fun snapshot(
        ...
        val schedule = dao.schedule(now)

        val dashboard = evolution.dashboard()

        val xpToday = evolution
            .history()
            .first()
            .filter {
                java.time.Instant
        ...
        val availableEvents = events.filter {
            !it.dismissedForever &&
                (it.snoozeUntil ?: 0L) <= now
        }
    ...
    suspend fun nextBestAction(
        ...
        val tasksNow = tasks.observeAll().first()

        val weakest = evolution
            .dashboard()
            .skills
            .minWithOrNull(
        ...
        return events
            .sortedWith(
                compareByDescending<ProactiveEvent> { event ->
                    ...
                    val fit = matchingTask?.let {
                        if (it.estimatedMinutes <= minutes) {
                            1.0
                        } else {
                            0.55
                        }
            ...
            .firstOrNull {
                free ||
                    it.priority ==
                    ProactivePriority.CRITICAL.name
            }
    }
    ...
    suspend fun complete(id: Long) {
        dao.event(id)?.let {
            dao.updateEvent(
                it.copy(
                    status =
                        ProactiveStatus.COMPLETED.name,
                    communicationState = "COMPLETED"
                )
            )

            learn(it.type, "follow")
        }
    }
    ...
    suspend fun createSubtasks(
        ...
        val skill = skillName?.let {
            evolution.findSkill(it)
        }

        steps.forEachIndexed { index, step ->
            tasks.insert(
                com.harsh.jarvis.tasks.Task(
                    title =
                        "$title — ${index + 1}. $step",
                    description =
                        "Subtask of: $title" +
            ...
            if (skill != null) {
                evolution.addQuest(
                    com.harsh.jarvis.evolution.EvolutionQuest(
                        title =
                            "$title — ${index + 1}. $step",
                        description =
    ...
    suspend fun dailyBriefing(): String {
        ...
        return "Good morning. You have ${tasksToday.size} task(s) due today and ${snapshot.important.size} important proactive item(s). Recommended priority: $recommended." +
    ...
    suspend fun eveningReview(): String {
        ...
        return "Evening review: $completed task(s) completed, " +
            "$incomplete incomplete, " +
            "${snapshot.xpToday} XP gained today, " +
            "$tomorrowDue deadline(s) tomorrow. " +
            if (weakest != null) {
                "Skill needing attention: $weakest. "
            } else {
                ""
            } +
            "Next priority: " +
            "${snapshot.recommendation?.title ?: "none"}."
    }
    ...
    suspend fun queueMessage(
        ...
        if (
            dao.byDedupeKey(event.dedupeKey) == null
        ) {
    ...
    private suspend fun upsert(
        ...
        val key =
            "$source:${sourceId ?: title.lowercase().trim()}"
        ...
        val priority =
            PriorityEngine.classify(
                ...
                existing?.manualPriority?.let {
                    runCatching {
                        ProactivePriority.valueOf(it)
                    }.getOrNull()
                }
            )
